...
- The shared Frostyard docs top bar, including search and the Source link.
...
- `TopBar.astro` for the established brand, search, and source controls.
...
1. `TopBar` with its existing search and Source controls.
...
- The top bar keeps its existing responsive behavior.
